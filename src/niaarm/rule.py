...
class Rule:
    ...
    Args:
        ...
        fitness (float | None): Fitness value of the association rule.
    ...
    Attributes:
        ...
        support: Support is defined on an itemset as the proportion of transactions
         ...
         For an association rule, support is defined as the support of all the
         ...
         **Reference:** Michael Hahsler, A Probabilistic Comparison of Commonly Used
         ...
         **Range:** :math:`[0, \infty]` (1 means independence, :math:`\infty` means
        ...
        inclusion: Inclusion is defined as the ratio between the number of attributes
         ...
         where :math:`m` is the total number of attributes in the transactional
         database.


         **Range:** :math:`[0, 1]`

         **Reference:** I. Fister Jr., V. Podgorelec, I. Fister. Improved
         Nature-Inspired Algorithms for Numeric Association
         Rule Mining. In: Vasant P., Zelinka I., Weber GW. (eds) Intelligent
        ...
        amplitude: Amplitude measures the quality of a rule, preferring attributes
         ...
         where :math:`n` is the total number of attributes in the rule, :math:`Ub_k`
         and :math:`Lb_k` are upper and lower
         bounds of the selected attribute, and :math:`max(o_k)` and :math:`min(o_k)`
         are the maximum and minimum
         feasible values of the attribute :math:`o_k` in the transactional database.

         **Range:** :math:`[0, 1]`

         **Reference:** I. Fister Jr., I. Fister A brief overview of swarm
        ...
        interestingness: Interestingness of the rule, defined as:
         ...
         :math:`interest(X \implies Y) = \frac{supp(X \implies Y)}{supp(X)} \cdot
         \frac{supp(X \implies Y)}{supp(Y)}
         \cdot (1 - \frac{supp(X \implies Y)}{|D|})`

         Here, the first part gives us the probability of generating the rule based
         on the antecedent, the second part
         gives us the probability of generating the rule based on the consequent and
         the third part is the probability
         that the rule won't be generated. Thus, rules with very high support will be
        ...
        netconf: The netconf metric evaluates the interestingness of
         ...
         :math:`netconf(X \implies Y) = \frac{supp(X \implies Y) - supp(X)supp(Y)}{
         supp(X)(1 - supp(X))}`

         **Range:** :math:`[-1, 1]` (Negative values represent negative dependence,
         positive values represent positive
         dependence and 0 represents independence)

         **Reference:** E. V. Altay and B. Alatas, "Sensitivity Analysis of MODENAR
         Method for Mining of Numeric Association
         Rules," 2019 1st International Informatics and Software Engineering
         ...
         **Range:** :math:`[-1, 1]` (-1 reflects total negative association,
         ...
         **Reference:** T. Zhang, “Association Rules,” in Knowledge Discovery and
         Data Mining. Current Issues and New
         ...
         **Reference:** Gregory Piatetsky-Shapiro. 1991. Discovery, Analysis,
         and Presentation of Strong Rules. In
         Knowledge Discovery in Databases, Gregory Piatetsky-Shapiro and William J.
         Frawley (Eds.). AAAI/MIT Press, 229–248.
    """
        ...
        for attribute in self.antecedent:
            if attribute.dtype != "cat":
                ...
                acc += (
                    ...
                    else (attribute.max_val - attribute.min_val)
                    / (feature_max - feature_min)
                )
                contains_antecedent &= (
                    (transactions[attribute.name] <= attribute.max_val)
                    & (transactions[attribute.name] >= attribute.min_val)
                ).to_numpy()
            else:
                contains_antecedent &= np.isin(
                    transactions[attribute.name], attribute.categories
                )
    ...
    def leverage(self):
        return self.support - (
            (self.antecedent_count / self.num_transactions)
            * (self.consequent_count / self.num_transactions)
        )
