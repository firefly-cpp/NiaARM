...
class Feature:
    ...
    Args:
       ...
       min_val (float | None): Minimum value of the feature in the transaction database.
       max_val (float | None): Maximum value of the feature in the transaction database.
       categories (list[str] | None): Possible categorical feature's values.
    ...
    def __repr__(self):
        ...
        if self.dtype == "cat":
            if len(self.categories) > 1:
                string += f"{self.categories})"
            else:
                string += f"{self.categories[0]})"
        else:
